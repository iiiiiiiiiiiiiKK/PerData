...
<head>
    ...
    <title>Binance Futures Pro</title>
...
<body>
    ...
    <script type="text/babel">
        ...
        // --- HELPERS ---
        const formatCurrency = (n) => typeof n === 'number' && !isNaN(n) ? '$' + Intl.NumberFormat('en-US', { notation: "compact", maximumFractionDigits: 1 }).format(n) : '-';
        const formatPriceReal = (n) => typeof n === 'number' && !isNaN(n) ? n.toLocaleString('en-US', { maximumFractionDigits: 10, useGrouping: false }) : '-';
        const formatDate = (s) => s ? new Date(s).toISOString().split('T')[0] : '-';
        const getListedCategory = (ts) => {
        ...
        const calcRangePos = (current, high, low) => {
        ...
        const getCountdown = (nextFundingTime) => {
        ...
        const EXPORT_COLUMNS = [
            { key: 'symbol', label: 'Symbol', subLabel: '[代币]', format: (v) => '$' + normalizeSymbol(v) },
            { key: 'onboardDate', label: 'Listed', subLabel: '[上线]', format: (v) => v ? new Date(v).toISOString().split('T')[0] : '-' },
            { key: 'price', label: 'Price', subLabel: '[价格]', format: formatPriceReal },
            { key: 'imbalance', label: 'Imbal%', subLabel: '[失衡]', format: (v) => v?(v*100).toFixed(2)+'%':'-' },
            { key: 'topDepth', label: 'Depth($)', subLabel: '[深度]', format: (v) => v?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) },
            { key: 'rangePosAll', label: 'Pos(All)', subLabel: '[历史位置]', format: (v) => v?.toFixed(0) + '%' },
            { key: 'floatRatio', label: 'Float %', subLabel: '[流通率]', format: (v) => v?.toFixed(1) + '%' },
            { key: 'coverTime', label: 'Cover (H)', subLabel: '[清算]', format: (v) => v?.toFixed(1) + 'h' },
            { key: 'basis', label: 'Basis', subLabel: '[基差]', format: (v) => v?(v>0?'+':'')+(v*100).toFixed(2)+'%':'-' },
            { key: 'avgTrade', label: 'AvgTrade', subLabel: '[均笔]', format: (v) => v?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) },
            { key: 'oiMcapRatio', label: 'OI/Mcap', subLabel: '[杠杆率]', format: (v) => v?.toFixed(3) },
            { key: 'volOiRatio', label: 'Vol/OI', subLabel: '[换手/持仓]', format: (v) => v?.toFixed(2) },
            { key: 'sma20Gap', label: 'SMA Gap', subLabel: '[均线偏离]', format: (v) => v?(v>0?'+':'')+(v*100).toFixed(2)+'%':'-' },
            { key: 'spread', label: 'Spread', subLabel: '[点差]', format: (v) => (v * 100)?.toFixed(3) + '%' },
            { key: 'volStrength', label: 'Vol Str', subLabel: '[量能]', format: (v) => v?.toFixed(2) + 'x' },
            { key: 'distHL', label: 'Dist H/L', subLabel: '[高低距离]', format: (v) => `${(v?.h*100).toFixed(1)}% / ${(v?.l*100).toFixed(1)}%` },
            { key: 'vwapGap', label: 'VWAP Gap', subLabel: '[均价偏离]', format: (v) => v?(v>0?'+':'')+(v*100).toFixed(2)+'%':'-' },
            { key: 'rsi', label: 'RSI(14D)', subLabel: '[强弱]', format: (v) => v?.toFixed(1) },
            { key: 'volatility', label: 'Vol(7D)', subLabel: '[波动]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'turnover', label: 'Turnover', subLabel: '[换手]', format: (v) => (v * 100)?.toFixed(2) + '%' },
            { key: 'trades', label: 'Trades', subLabel: '[笔数]', format: (v) => v?.toLocaleString('en-US') },
            { key: 'rangePos', label: 'Pos %', subLabel: '[位置]', format: (v) => v?.toFixed(0) + '%' },
            { key: 'fundingRate', label: 'Funding', subLabel: '[费率]', format: (v) => (v * 100).toFixed(4) + '%' },
            { key: 'openInterest', label: 'OI (USD)', subLabel: '[持仓]', format: (v) => v?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) },
            { key: 'volume', label: 'Volume', subLabel: '[成交]', format: (v) => v?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) },
            { key: 'changePercent1h', label: '1h %', subLabel: '[1时]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'changePercent4h', label: '4h %', subLabel: '[4时]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'changePercent24h', label: '24h %', subLabel: '[24时]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'changePercent7d', label: '7d %', subLabel: '[7天]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'changePercent30d', label: '30d %', subLabel: '[30天]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'hasSpot', label: 'Spot', subLabel: '[现货]', format: (v) => v ? 'Yes' : 'No' },
            { key: 'mcap', label: 'Mcap', subLabel: '[市值]', format: (v) => v?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) },
            { key: 'fdv', label: 'FDV', subLabel: '[全稀释]', format: (v) => v?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) },
            { key: 'dominance', label: 'Dom %', subLabel: '[占比]', format: (v) => v?.toFixed(2) + '%' },
            { key: 'volMcapRatio', label: 'V/Mc', subLabel: '[量值比]', format: (v) => v?.toFixed(2) },
            { key: 'circulating', label: 'Circ', subLabel: '[流通]', format: (v) => v?.toLocaleString('en-US') },
            { key: 'totalSupply', label: 'Total', subLabel: '[总量]', format: (v) => v?.toLocaleString('en-US') },
            { key: 'ath', label: 'ATH', subLabel: '[最高]', format: formatPriceReal },
            { key: 'athDrop', label: 'ATH Drop', subLabel: '[回撤]', format: (v) => v?.toFixed(2) + '%' },
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (data && Array.isArray(data.symbols)) {
                            const spotSet = new Set(data.symbols.filter((s) => s.status === 'TRADING').map((s) => s.symbol));
        ...
        const enqueueOI = useCallback((symbols) => {
        ...
        const updateDisplayData = () => {
            ...
            tickerMapRef.current.forEach((val, key) => {
        ...
        const initBinance = async () => {
            try {
                ...
                if (res && Array.isArray(res.symbols)) {
                    ...
                    res.symbols.forEach((s) => dateMap.set(s.symbol, s.onboardDate));
        ...
        const initCoinGecko = async () => {
            try {
                ...
                if (Array.isArray(list)) list.forEach((c) => symbolToIdRef.current.set(c.symbol.toUpperCase(), c.id));
        ...
        const queueDetails = async (symbol, cgId) => {
            ...
            try {
                ...
                if(item) {
                    ...
                    if (Array.isArray(k) && k.length>7) {
                        ...
                        const sl = k.map((x) => parseFloat(x[4]));
                        const vols = k.slice(k.length-8, k.length-1).map((x) => parseFloat(x[7])); 
                        if (sl.every((n) => !isNaN(n))) {
                            item.sparkline = sl;
            ...
            <div className="flex-1 min-h-0 relative">
                {isLoaded ? (
                    <VirtualTable 
                        ...
                        toggleFav={(s) => setFavorites(prev => { const n = new Set(prev); n.has(s)?n.delete(s):n.add(s); return n; })}
        ...
        // --- CONFIG MODAL ---
        const ColumnConfigModal = ({ onClose, currentOrder, onSave, currentWidths, onSaveWidths, t }) => {
            ...
            const getLabel = (id) => EXPORT_COLUMNS.find(c => c.key === id)?.label || id;
        ...
        // --- EXPORT MODAL ---
        const ExportModal = ({ onClose, data, t }) => {
            ...
            const generatePlainText = () => {
                ...
                const body = rows.map((r) => r.map((cell, i) => String(cell).padEnd(colWidths[i])).join('   ')).join('\n');
                return `${getHeaderDescription()}${header}\n${separator}\n${body}`;
            };
        ...
        const PctCell = ({ v, box, isPixel, theme }) => (
        ...
        const VirtualTable = ({ data, theme, t, favorites, toggleFav, toggleSpot, spotFilter, onRequestDetails, onLoadStats, onVisibleSymbols, columnOrder, columnWidths, setColumnWidths }) => {
            ...
            const visibleItems = sortedData.slice(renderStart, renderEnd).map((item, index) => ({
                ...item,
            ...
            useEffect(() => {
                const timer = setTimeout(() => {
                    const symbols = visibleItems.map((i) => i.symbol);
            ...
            const COLUMN_DEFS = {
                onboardDate: { label: 'Listed', subLabel: '[上线]', render: (item) => { const { label, color } = getListedCategory(item.onboardDate); return <div className="w-full text-center flex justify-center"><span className={`px-1 rounded border ${color}`}>{label}</span></div>; } },
                price: { label: 'Price', subLabel: '[价格]', render: (item) => <div className="w-full text-right px-1 font-mono">{formatPriceReal(item.price)}</div> },
                imbalance: { label: 'Imbal%', subLabel: '[失衡]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.imbalance>0.2?'text-green-500':item.imbalance<-0.2?'text-red-500':''}`}>{item.imbalance?(item.imbalance*100).toFixed(2)+'%':'-'}</div> },
                topDepth: { label: 'Depth($)', subLabel: '[深度]', render: (item) => <div className={`w-full text-right px-1 font-mono`}>{item.topDepth?.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })}</div> },
                rangePosAll: { label: 'Pos(All)', subLabel: '[历史位置]', render: (item) => <RangeBar pos={item.rangePosAll} /> },
                floatRatio: { label: 'Float %', subLabel: '[流通率]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.floatRatio>90?'text-green-500':'opacity-60'}`}>{item.floatRatio?.toFixed(1)}%</div> },
                coverTime: { label: 'Cover (H)', subLabel: '[清算]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.coverTime>24?'text-red-500':''}`}>{item.coverTime?.toFixed(1)}h</div> },
                basis: { label: 'Basis', subLabel: '[基差]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.basis>0.001?'text-red-500':item.basis<-0.001?'text-green-500':''}`}>{item.basis?(item.basis*100).toFixed(2)+'%':'-'}</div> },
                avgTrade: { label: 'Avg Trade', subLabel: '[均笔]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.avgTrade>5000?'text-yellow-500 font-bold':''}`}>{item.avgTrade?formatCurrency(item.avgTrade):'-'}</div> },
                rangePos: { label: 'Pos', subLabel: '[位置]', render: (item) => <RangeBar pos={item.rangePos} /> },
                trend: { label: 'Trend(7d)', subLabel: '[趋势]', sortable: false, render: (item) => <div className="w-full h-full flex items-center justify-center px-1"><Sparkline data={item.sparkline} /></div> },
                rsi: { label: 'RSI(14D)', subLabel: '[强弱]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.rsi>70?'text-red-500':item.rsi<30?'text-green-500':'opacity-60'}`}>{item.rsi?.toFixed(1)||'-'}</div> },
                volatility: { label: 'Vol(7D)', subLabel: '[波动]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.volatility>5?'text-orange-500':''}`}>{item.volatility?.toFixed(2)}%</div> },
                trades: { label: 'Trades', subLabel: '[笔数]', render: (item) => <div className="w-full text-right px-1 font-mono">{item.count?.toLocaleString()}</div> },
                spread: { label: 'Spread', subLabel: '[点差]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.spread>0.001?'text-red-500':'text-green-500'}`}>{(item.spread*100)?.toFixed(3)}%</div> },
                volStrength: { label: 'Vol Str', subLabel: '[量能]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.volStrength>2?'text-yellow-500 font-bold':''}`}>{item.volStrength?.toFixed(2)}x</div> },
                distHL: { label: 'Dist H/L', subLabel: '[高低]', sortable: false, render: (item) => <div className="w-full text-right px-1 font-mono text-[9px]"><span className="text-red-500">{(item.distHL?.h*100).toFixed(1)}%</span> / <span className="text-green-500">{(item.distHL?.l*100).toFixed(1)}%</span></div> },
                vwapGap: { label: 'VWAP Gap', subLabel: '[偏离]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.vwapGap>0.02?'text-red-500':item.vwapGap<-0.02?'text-green-500':''}`}>{(item.vwapGap*100)?.toFixed(2)}%</div> },
                turnover: { label: 'Turnover', subLabel: '[换手]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.turnover>0.5?'text-yellow-500 font-bold':''}`}>{(item.turnover*100)?.toFixed(1)}%</div> },
                fundingRate: { label: 'Fund/Time', subLabel: '[费率]', render: (item) => <div className={`w-full text-right px-1 font-mono flex flex-col justify-center`}><span className={item.fundingRate>0?'text-green-500':item.fundingRate<0?'text-red-500':''}>{item.fundingRate?(item.fundingRate*100).toFixed(4)+'%':'-'}</span><span className="text-[8px] opacity-50">{getCountdown(item.nextFundingTime)}</span></div> },
                openInterest: { label: 'OI(USD)', subLabel: '[持仓]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{item.openInterest ? formatCurrency(item.openInterest) : <span className="opacity-30">...</span>}</div> },
                volume: { label: 'Vol', subLabel: '[成交]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{formatCurrency(item.volume)}</div> },
                changePercent1h: { label: '1h', subLabel: '[1时]', render: (item) => <PctCell v={item.changePercent1h} isPixel={isPixel} theme={theme} /> },
                changePercent4h: { label: '4h', subLabel: '[4时]', render: (item) => <PctCell v={item.changePercent4h} isPixel={isPixel} theme={theme} /> },
                changePercent24h: { label: '24h', subLabel: '[24时]', render: (item) => <PctCell v={item.changePercent24h} box isPixel={isPixel} theme={theme} /> },
                changePercent7d: { label: '7d', subLabel: '[7天]', render: (item) => <PctCell v={item.changePercent7d} isPixel={isPixel} theme={theme} /> },
                changePercent30d: { label: '30d', subLabel: '[30天]', render: (item) => <PctCell v={item.changePercent30d} isPixel={isPixel} theme={theme} /> },
                hasSpot: { label: 'Spot', subLabel: '[现货]', render: (item) => <div className="w-full text-center">{item.hasSpot?<span className="text-green-500">✓</span>:<span className="text-gray-300">·</span>}</div> },
                mcap: { label: 'Mcap', subLabel: '[市值]', render: (item) => <div className="w-full text-right px-1 font-mono">{formatCurrency(item.mcap)}</div> },
                fdv: { label: 'FDV', subLabel: '[全稀释]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{formatCurrency(item.fdv)}</div> },
                dominance: { label: 'Dom%', subLabel: '[占比]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{item.dominance?.toFixed(2)}%</div> },
                volMcapRatio: { label: 'V/Mc', subLabel: '[量值比]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{item.volMcapRatio?.toFixed(2)}</div> },
                circulating: { label: 'Circ', subLabel: '[流通]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{formatCurrency(item.circulating)}</div> },
                totalSupply: { label: 'Tot', subLabel: '[总量]', render: (item) => <div className={`w-full text-right px-1 font-mono ${t.textSub}`}>{formatCurrency(item.totalSupply)}</div> },
                ath: { label: 'ATH', subLabel: '[最高]', render: (item) => <div className="w-full text-right px-1 font-mono flex flex-col justify-center"><span>{formatPriceReal(item.ath)}</span>{item.athDrop !== null && !isNaN(item.athDrop) && (<span className={`text-[8px] ${item.athDrop < -80 ? 'text-red-500 font-bold' : 'opacity-50'}`}>{item.athDrop.toFixed(0)}%</span>)}</div> },
                atl: { label: 'ATL', subLabel: '[最低]', render: (item) => <div className="w-full text-right px-1 font-mono flex flex-col justify-center"><span>{formatPriceReal(item.atl)}</span><span className="text-[8px] opacity-50">{formatDate(item.atlDate)}</span></div> },
                sector: { label: 'Sector', subLabel: '[板块]', render: (item) => <div className="w-full text-right px-1 truncate">{item.sector||'-'}</div> },
                website: { label: 'Web', subLabel: '[官网]', sortable: false, render: (item) => <div className="w-full text-center flex justify-center">{item.website?<a href={item.website} target="_blank"><IconGlobe className="w-3 h-3"/></a>:'-'}</div> },
                description: { label: 'Desc', subLabel: '[描述]', sortable: false, render: (item) => <div className="w-full text-left truncate opacity-70">{item.description||'-'}</div> },
                oiMcapRatio: { label: 'OI/Mcap', subLabel: '[杠杆率]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.oiMcapRatio>0.5?'text-red-500':''}`}>{item.oiMcapRatio?.toFixed(3)}</div> },
                volOiRatio: { label: 'Vol/OI', subLabel: '[换手/持仓]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.volOiRatio>5?'text-yellow-500':''}`}>{item.volOiRatio?.toFixed(2)}</div> },
                sma20Gap: { label: 'SMA Gap', subLabel: '[均线偏离]', render: (item) => <div className={`w-full text-right px-1 font-mono ${item.sma20Gap>0.1?'text-red-500':item.sma20Gap<-0.1?'text-green-500':''}`}>{item.sma20Gap?(item.sma20Gap*100).toFixed(2)+'%':'-'}</div> },
            };

            return (
                <div className={`w-full h-full overflow-auto scroll-container flex flex-col ${t.containerClass}`} ref={containerRef} onScroll={e => setScrollTop((e.target).scrollTop)}>
